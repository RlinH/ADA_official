...
class BezierTrans(nn.Module):
    ...
    def forward(self, batch_images):
        ...
        bezier_curve = self.compute_bezier_curve(t_values, P1, P2, batch_size)
...
class ControlShift(nn.Module):
    def __init__(self):
        ...
        # 初始化fc层的权重和偏置为0
        nn.init.zeros_(self.fc.weight)  # 将权重初始化为0
...
#         x =  standardized_images
        ...
        x = self.beziertrans(x)
        #x = self.conshift(x)
#         normalized_aug_images = torch.zeros_like(x)
#         for i in range(x.shape[0]):
#             img_mean = x[i].mean()
#             img_std = x[i].std()
#             normalized_aug_images[i] = (x[i] - img_mean) / (img_std + 1e-8)

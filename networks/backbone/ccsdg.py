...
class CCSDG(nn.Module):
    def __init__(self, output_stride=8, BatchNorm=None, width_mult=1., pretrained=True):
        ...
        self.low_level_features = self.features[1:4]
    
        self.high_level_features = self.features[4:]
        
        self.first_layer = self.features[0]
        self.channel_prompt = nn.Parameter(torch.randn(2, 32, 1, 1))
    ...
    def forward(self, x , tau =0.1):
        x = self.first_layer(x) #([batch, 32, h/2, w/2])
        channel_prompt_onehot = torch.softmax(self.channel_prompt / tau, dim=0)# ([2, 32, 1, 1])
        f_content = x * channel_prompt_onehot[0].view(1, *channel_prompt_onehot[0].shape) #([batch, 32, h/2, w/2])
        f_style = x * channel_prompt_onehot[1].view(1, *channel_prompt_onehot[1].shape)#([batch, 32, h/2, w/2])
        x = F.relu(f_content)
...
if __name__ == "__main__":
    input = torch.rand(8, 3, 500, 500)
    model = CCSDG(output_stride=16, BatchNorm=nn.BatchNorm2d)

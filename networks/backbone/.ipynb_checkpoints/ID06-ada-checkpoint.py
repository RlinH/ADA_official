...
class ControlShift(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # MLP接受卷积后特征输入，输出6个参数（确保全连接层的输入维度是4 * in_channels）
        self.fc = nn.Linear(in_channels * 4, 6)  # 四个特征源：1x1, 3x3, 5x5 和 avg_pool

    def forward(self, image):
        # 应用平均池化
        avg_pooled = self.avg_pool(image)
        avg_pooled = torch.flatten(avg_pooled, 1)

        # 应用多尺度卷积
        conv1x1 = self.conv1x1(image)
        conv3x3 = self.conv3x3(image)
        conv5x5 = self.conv5x5(image)

        # 全局平均池化提取特征
        f1 = torch.mean(conv1x1, dim=[2, 3])
        f2 = torch.mean(conv3x3, dim=[2, 3])
        f3 = torch.mean(conv5x5, dim=[2, 3])

        # 合并来自不同卷积核的特征
        x = torch.cat((f1, f2, f3, avg_pooled), dim=1)

...
class ADA(nn.Module):
    ...
    def forward(self, image):
        ...
        normalized_images = torch.zeros_like(image)
        for i in range(image.shape[0]):
            img_min = image[i].min()
            img_max = image[i].max()
            normalized_images[i] = (image[i] - img_min) / (img_max - img_min + 1e-8)

        aug_x = self.conshift(normalized_images)
        
        normalized_aug_images = torch.zeros_like(aug_x)
        for i in range(aug_x.shape[0]):
            img_min = aug_x[i].min()
            img_max = aug_x[i].max()
            normalized_aug_images[i] = (aug_x[i] - img_min) / (img_max - img_min + 1e-8)
        
        low_level_feat = self.low_level_features(normalized_aug_images)

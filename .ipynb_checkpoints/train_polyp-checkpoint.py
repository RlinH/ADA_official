...
def step_decay_lr(epoch, initial_lr, drop_rate, epochs_drop):
    return initial_lr * (drop_rate ** (epoch // epochs_drop))

def adjust_learning_rate_step(optimizer, epoch, initial_lr, drop_rate, epochs_drop):
    lr = step_decay_lr(epoch, initial_lr, drop_rate, epochs_drop)
...
class Trainer(object):
    ...
    def __init__(self, cuda, multiply_gpu, model, optimizer, scheduler, val_loader, domain_loader, out, max_epoch, stop_epoch=None,
        ...
        self.optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.99, nesterov=True)
    ...
    def validate_prostate(self,val_loader=None,dataset_name= None):
        ...
        with torch.no_grad():
            for batch_idx, sample in enumerate(val_loader):
                ...
                dice_score = dice_coeff(predictions, target_map)
                total_dice += dice_score.item()
                iou_score = iou_coeff(predictions, target_map)
            ...
            self.writer.add_scalar(f'val_data/avg_dice/{dataset_name}', avg_dice, self.epoch )
            self.writer.add_scalar(f'val_data/avg_iou/{dataset_name}', avg_iou, self.epoch )
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.max_epoch), desc='Training Progress'):
            lr = adjust_learning_rate(self.optimizer, self.epoch, initial_lr=0.01, max_epochs=300)
            
            #lr = adjust_learning_rate_cosine(self.optimizer, self.epoch, initial_lr=0.001, max_epochs=200)
            
            #lr = adjust_learning_rate_step(self.optimizer, self.epoch, initial_lr=0.01, drop_rate=0.95, epochs_drop=1)
            self.epoch = epoch
            ...
            if (self.epoch) >100:
                #self.test_prostate()
                self.validate_prostate(self.val_loader,'Kvasir-SEG')
                self.validate_prostate(self.domain2_loader,'CVC-ColonDB')
                
        self.writer.close()

args.EXPID = "POLYP-RSC0.05-Epoch300-poly_lr0.9-NoRSC-repeat-0806v2"
...
if __name__ == '__main__':
    ...
    now = datetime.now()
    for x in ['CVC-ClinicDB']:#
        args.dataset = x
        args.out = osp.join('logs_train','Visualization','ADA',args.dataset, args.method,current_time)
        ...
        trainset = PolypDataset(img_height=352, img_width=352, dataset=args.dataset, base_dir='./dataset/polyp', split='train', transform=train_transform,method =args.method)

        trainloader = DataLoader(trainset, batch_size=8, num_workers=10,
                             shuffle=True, drop_last=True, pin_memory=True, collate_fn=collate_fn_tr_styleaug)

        testset = PolypDataset(img_height=352, img_width=352, dataset='Kvasir-SEG', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)
        ...
        testset2 = PolypDataset(img_height=352, img_width=352, dataset='CVC-ColonDB', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)
        ...
        testset = PolypDataset(img_height=352, img_width=352, dataset='CVC-ClinicDB', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)

        testloader = DataLoader(testset, batch_size=8, num_workers=0,
                             shuffle=False, drop_last=False, pin_memory=True, worker_init_fn=seed_worker, collate_fn=collate_fn_ts)

        testset = PolypDataset(img_height=352, img_width=352, dataset='CVC-ColonDB', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)

        testloader2 = DataLoader(testset, batch_size=8, num_workers=0,
                             shuffle=False, drop_last=False, pin_memory=True, worker_init_fn=seed_worker, collate_fn=collate_fn_ts)
        testset = PolypDataset(img_height=352, img_width=352, dataset='ETIS-LaribPolypDB', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)

        testloader3 = DataLoader(testset, batch_size=8, num_workers=0,
                             shuffle=False, drop_last=False, pin_memory=True, worker_init_fn=seed_worker, collate_fn=collate_fn_ts)

        testset = PolypDataset(img_height=352, img_width=352, dataset='Kvasir-SEG', base_dir='./dataset/polyp', split='test', transform=test_transform,method =args.method)

        testloader4 = DataLoader(testset, batch_size=8, num_workers=0,
                             shuffle=False, drop_last=False, pin_memory=True, worker_init_fn=seed_worker, collate_fn=collate_fn_ts)
